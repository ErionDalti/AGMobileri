...
class NetworkHelper {
  ...
  Future<dynamic> placeOrder(
    ...
    if (response.statusCode == 201) {
      ...
      //   convert.jsonDecode(response.body),
      // );
      print("Payment Inside request data:");
      log(response.body);
      return convert.jsonDecode(response.body);
    } else {
      await _handleOtherCases(response, context);
  ...
  Future<Map<String, String>> _getHeaders(BuildContext context) async {
    ...
    if (await SharedPrefUtil.contains(kKeyCurrency)) {
      ...
      var data = await SharedPrefUtil.getString(kKeyCurrency);
    }
  ...
  Future<Map<String, String>> _getHeadersWithoutContext() async {
    HashMap<String, String> headers = HashMap();

    headers[HttpHeaders.acceptHeader] = kResponseOfJsonType;
    headers[kKeyHeaderLanguage] = 'en';
    if (await SharedPrefUtil.contains(kKeyCurrency)) {
      headers[kKeyHeaderCurrency] =
          await SharedPrefUtil.getString(kKeyCurrency);
      var data = await SharedPrefUtil.getString(kKeyCurrency);
    ...
    const String accesstokensi =
        '%3D';
    if (await SharedPrefUtil.contains(accesstokensi)) {
      headers[HttpHeaders.authorizationHeader] =
          await SharedPrefUtil.getString(accesstokensi);
      debugPrint('token' + await SharedPrefUtil.getString(accesstokensi));
    }
  ...
  Future<http.Response> _getRequestWithoutContext({
    String url,
  }) async {
    http.Response response;
    ...
    try {
      response = await http.get(
        Uri.parse(url),
        headers: await _getHeadersWithoutContext(),
      );
      //debugPrint(url + response.body);
    } on SocketException {
      ToastUtil.show("Please check your internet connection");
  ...
  Future getColorTypesByProductId(productId) async {
    http.Response response;
    String url = kApiGetAttributesByProductId + '/' + productId;
    print(url);
    response = await _getRequestWithoutContext(
      url: url,
    );
  ...
  Future getVariantByProductId(productId, color) async {
    http.Response response;
    String url = kApiGetVariantByProductId + '/' + productId + '/' + color;
    print(url);
    response = await _getRequestWithoutContext(
      url: url,
    );
