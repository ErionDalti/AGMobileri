...
import 'package:azzoa_grocery/data/local/model/VariantProductByColorModel.dart';
...
class _ProductDetailsPageState extends State<ProductDetailsPage> {
  ...
  List<AttributeColorModel> colors = [];
  // [
  //   "#ff0000",
  //   "#00FFFF",
  //   "#000000",
  ...
  void didChangeDependencies() {
    ...
    _getAllColorAttributes(widget.productId)
                  ...
                  child: new ListView(
                    ...
                    children: colors
                        .map<Widget>((color) => GestureDetector(
                              ...
                              onTap: () {
                                _getVariantByColorAttribute(
                                        widget.productId, color.slug)
                                    .then((value) => setState(() {
                                          print(value.length);
                                          print("erdhi request");
                                          item.gallery = value;
  ...
  Future<List<AttributeColorModel>> _getAllColorAttributes(productId) async {
    List<AttributeColorModel> colorModels = [];
    try {
      print("The data:");
      var result = await NetworkHelper.on().getColorTypesByProductId(productId);
      var colorAttributes = json.decode(result);

      print(colorAttributes);

      AttributeColors attr = AttributeColors.fromJson(colorAttributes);
      print("Kaloi convertin");
      if (attr.data != null) {
        if (attr.data.json_array != null) {
          attr.data.json_array.forEach((element) {
            var color = new AttributeColorModel();
            color.id = element.id;
            color.attribute_id = element.attribute_id;
            color.data = element.data;
            color.slug = element.slug;
            color.created_at = element.created_at;
            color.updated_at = element.updated_at;
            color.name = element.name;
            colorModels.add(color);
          });
          print("Color name :");
          //print(colorModels[0].name);
        }
      }
      return colorModels;
    } catch (e) {
...
Future<List<Gallery>> _getVariantByColorAttribute(productId, color) async {
  List<Gallery> galleryList = [];
  try {
    print("Variant Data:");
    var result =
        await NetworkHelper.on().getVariantByProductId(productId, color);
    var colorAttributes = json.decode(result);
    print("Para convertit:");
    VariantByColor attr = VariantByColor.fromJson(colorAttributes);
    print("Kaloi convertin");
    if (attr.data != null) {
      if (attr.data.json_object != null) {
        attr.data.json_object.gallery.forEach((element) {
          Gallery galery = new Gallery();
          galery.url = element.url;
          galery.thumbUrl = element.thumb_url;
          galleryList.add(galery);
        });
      }
    }
    print("Mbaroi Cikli:");
